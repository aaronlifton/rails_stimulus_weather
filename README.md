...
Open `http://localhost:3000` and enter an address (Preferrably city + state).
...
- `lat`: latitude (float, 7-decimal rounding)
- `long`: longitude (float, 7-decimal rounding)
- `zipcode`: string; census API may omit it
...
- `temperature_current`: current temperature in Fahrenheit (OpenWeatherMap `imperial` units)
...
- `code`: symbolic error code for UI presentation
- `reasons`: optional list (for API error messages)

- Errors are modeled as custom exception types (`GeocoderApi::Error`, `WeatherApi::Error`), which helpers keep controller logic straightforward.
